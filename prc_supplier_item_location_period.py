...
#working tables
result_supplier_item_location_period='result_supplier_item_location_period'


property_norm = "property_norm"
supplier_item_location_period_norm = "supplier_item_location_period_norm"
supplier_item_location_period = "supplier_item_location_period"
condition = "condition"
unit = "unit"
...
def extract_data_for_procedure(spark: SparkSession):
    ...
    dfs = {
        ...
        "supplier_item_location_period_norm": read_jdbc_data(supplier_item_location_period_norm, spark),
        "supplier_item_location_period": read_jdbc_data(supplier_item_location_period, spark),
...
def find_intersections(
        supplier_item_location_period_norm_df: DataFrame,
        property_norm_df: DataFrame,
        metric_id: int
) -> DataFrame:
    """
    Finds all valid intersections between scope data и properties.
    This function is PySpark-version of CTE 'preres' from procedure prc_reversal_supplier_item_location_period.
    """

    scope = supplier_item_location_period_norm_df.alias("scope")
...
def calculate_final_periods(intersections_df: DataFrame) -> DataFrame:
    ...
    window functions in SQL procedure prc_supplier_item_location_period.
    """

    scope_pk_cols = ["item_id", "location_id", "supplier_id"]
...
def calculate_gaps(final_periods_df: DataFrame, original_scope_df: DataFrame) -> DataFrame:
    ...
    This function is PySpark-version of the 2nd INSERT-block from SQL procedure prc_reversal_supplier_item_location_period.
    """
    scope_pk_cols = ["item_id", "location_id", "supplier_id"]
...
def find_unmatched_scopes(final_periods_df: DataFrame, original_scope_df: DataFrame) -> DataFrame:
    ...
    This function is PySpark-version of the 3rd INSERT-block from SQL procedure prc_reversal_supplier_item_location_period.
    """
    scope_pk_cols = ["item_id", "location_id", "supplier_id"]
...
def assemble_final_result(
    ...
    THis function is PySpark-version of the final SELECT-block from SQL procedure prc_reversal_supplier_item_location_period.
    """
    scope_pk_cols = ["item_id", "location_id", "supplier_id"]
...
def main(current_metric_id):
    ...
    intersections_df = find_intersections(
        supplier_item_location_period_norm_df=dfs["supplier_item_location_period_norm"],
    ...
    gaps_df = calculate_gaps(final_periods_df, dfs["supplier_item_location_period"])
    print("INFO: Step 5 results: Found 'gaps' (periods without property)")

    unmatched_df = find_unmatched_scopes(final_periods_df, dfs["supplier_item_location_period"])
    ...
    print("INFO: Step 7: Writing final result_supplier_item_location_period to PostgreSQL...")
    write_jdbc_data(final_result_df, result_supplier_item_location_period)
